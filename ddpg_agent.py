...
class Agents():
    ...
    def __init__(self, state_size, action_size, random_seed, n_agents=2):
        ...
        # Create one critic network for each agent
        self.critic_local = [Critic(state_size, action_size, random_seed).to(device) for i in range(n_agents)]
        self.critic_target = [Critic(state_size, action_size, random_seed).to(device) for i in range(n_agents)]
        self.critic_optimizer = [optim.Adam(self.critic_local[i].parameters(), lr=LR_CRITIC, weight_decay=WEIGHT_DECAY) for i in range(n_agents)]
    ...
    def learn_from_replay(self, agent):
